...
class XKCD(Lego):
    ...
    def handle(self, message):
        ...
        logger.info('Retrieving URL: {0!s}'.format(url))
        ...
        if webpage.status_code == requests.codes.ok:
            ...
            logger.error('HTTP GET response code: {0!s}'.format(webpage.status_code))
    ...
    def _parse_args(self,message):
        ...
        try:
            ...
            logger.debug('Found an argument: {0!s}'.format(str(id)))
        except IndexError:
            comic_id = None
            logger.debug('No args provided. Setting "id" to None')
        logger.debug('_parse_args comic_id: {0!s}'.format(comic_id))
        return comic_id
    ...
    def _build_url(self,comic_id):
        if comic_id is not None:
            if comic_id == 'r' or comic_id == 'random':
                ...
                logger.debug('User requested comic by id: {0!s}'.format(str(comic_id)))
                url = 'http://xkcd.com/{0!s}'.format(str(comic_id))
        else:
            url = 'http://xkcd.com/'
        return url
    ...
    def _parse_for_comic(self,r):
        ...
        if comic:
            ...
            response = "{0!s} {1!s}".format(altText, "http:" + comic.group(1))
        else:
