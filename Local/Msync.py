...
class Audit(Lego):
    ...
    def handle(self,message):
        ...
        if len(args) > 1:
            ...
            command = 'version'
        logger.debug('{0!s} requested. Dispatching {1!s} with args: {2!s}'.format(command,
    ...
    def _get_single_version(self,*args,**kwargs):
        ...
        if msync_ver.status_code == requests.codes.ok:
            ...
            logger.debug('Got a bad return code from module {0!s}: {1!s}'.format(module, msync_ver.status_code))
            return '0.0.0'

    def _generate_report(self,modules,*args):
        # input is a list. we want one module per line
        query = ' '.join(args)
        count = len(modules)
        timestamp = date.today()
        header = '# Report generated by TheVoxFox on {0!s}'.format(timestamp.strftime("%A, %d %B %Y"))
        modules.insert(0,header) 
        modules.insert(1,"Query: {0!s}".format(query))
        modules.insert(2,"{0!s} modules found".format(count))
        report = '\n'.join(modules)
        return report
    ...
    def _gist(self,text):
        ...
        logger.debug('_gist response code: {0!s}'.format(r.status_code))
        r = r.json()
        logger.debug(r)
        if 'html_url' in r:
            return r['html_url']
